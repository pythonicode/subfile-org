...
function formatTo08x(x: number): string {
    // formatted in hexadecimal ("%08x", x)
    return ("0000000" + ((x | 0) + 4294967296).toString(16)).substring(-8)
}
...
export function encode_mnemonic(message: string) {
    let out = []
    let shift = 0
    for (let i = 0; i < message.length / 8; i++) {
        ...
        let w1 = Math.floor(x % N)
        let w2 = Math.floor((x / N + w1) % N)
        let w3 = Math.floor((x / N / N + w2) % N)
...
export function decode_mnemonic(wlist: string[]) {
    if (wlist.length == 0) throw "No mnemonic provided as input."
    let out = ''
    const shift_word = wlist.shift() || "";
    console.log(shift_word);
    const shift = getWordIndex(shift_word) % 8;
    for (let i = 0; i < wlist.length / 3; i++) {
        const [word1, word2, word3] = wlist.slice(3 * i, 3 * i + 3)
        const w1 = getWordIndex(word1)
        const w2 = getWordIndex(word2) % N
        const w3 = getWordIndex(word3) % N
